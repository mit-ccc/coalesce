...
class MongoDB:
    ...
    def add_user(self, user_id):
        ...
        user_doc = {"user_id": user_id, "projects": [], "invite_token": {}}
    ...
    def store_invite_token(self, token, user_id, expiration):
        """
        Stores an invite token in the user's document.
        """
    ...
    def get_invite_token_info_for_user(self, user_id):
        """
        Retrieves invite token info for a specific user.
        """
        user = self.db.users.find_one({"user_id": user_id})
        if user and "invite_token" in user:
            return user["invite_token"]
        return None

    def get_invite_token_info(self, token):
        """
        Retrieves invite token info from the user's document.
        """
        user = self.db.users.find_one({"invite_token.token": token})
        if user and "invite_token" in user:
            return {
                "user_id": user["user_id"],
                "expiration": user["invite_token"]["expiration"]
            }
        return None

    def delete_invite_token(self, token):
        """
        Deletes the invite token from the user's document.
        """
        self.db.users.update_one(
    ...
    def format_project_for_export(self, project_id):
        """
        Formats a project for export as JSON to follow the CCC structure
        
        Returns a JSON-serializable dictionary containing all project data
        """
        # Use existing save_project_to_file method which already does most of the work
        project = self.save_project_to_file(project_id)
        ...
        new_project["type"] = project["context_response"]["4"]["response"].lower() # or template?
        new_project["ai_assisted_generation"] = True
        new_project["facilitated"] = False if project["context_response"]["4"]["response"].lower() == "survey" else True # or involves_facilitator
        new_project["status"] = "Final"
        new_project["context"] = project["context_response"]
        new_project["analytics"] = project["analyze_topics_info"]

        # Add the additional fields that will be blank (for now)
        new_project["description"] = ""
        new_project["visibility"] = ""
        new_project["media"] = []

        # Deal with the sections and prompts
        new_project["sections"] = [] # supposed to populate the Convo Guide Section collection
        for section in project["sections_with_cells"]:
            new_section = {}
            new_section["title"] = section["title"]
            new_section["order_index"] = section["id"]
            new_section["prompts"] = [] # supposed to populate the Convo Guide Prompt collection
            ...
            for i, cell in enumerate(section["cells"]):
                ...
                new_cell["prompt"] = cell["cell_details"]["main_text"]
                new_cell["prompt_type"] = cell["cell_details"]["cell_type"] # re-vist later
                new_cell["objective"] = cell["cell_details"]["description"] if "description" in cell["cell_details"] else ""
                new_cell["order_index"] = i
                new_cell["response_format"] = cell["cell_details"]["response_format"] if "response_format" in cell["cell_details"] else ""
                new_cell["response_categories"] = cell["cell_details"]["response_categories"] if "response_categories" in cell["cell_details"] else []
                new_cell["min_time_required"] = cell["time_estimate"] * 60
                new_cell["max_time_suggested"] = cell["time_estimate"] * 60

                # Add the additional section fields that will be blank (for now)
                new_cell["short_prompt"] = ""
                new_cell["category"] = ""
                new_cell["media"] = []
                new_cell["instructions"] = ""
                new_cell["script"] = ""

                new_section["prompts"].append(new_cell)
                # Add the time estimates
                total_min_time += cell["time_estimate"] * 60
                total_max_time += cell["time_estimate"] * 60

            # Calculate time estimates
            new_section["min_time_required"] = total_min_time
            new_section["max_time_suggested"] = total_max_time

            # Add the additional section fields that will be blank (for now)
            new_section["description"] = ""
            new_section["instructions"] = ""
