...
load_dotenv()  # noqa: E402 — must run before app-level imports that read env vars

from celery import Celery, Task, shared_task  # noqa: E402
from celery.result import AsyncResult  # noqa: E402
from db import MongoDB  # noqa: E402
from dspy_accessor import DSPyAccessor, DSPyModule  # noqa: E402
from flask import Flask, jsonify, request  # noqa: E402
from flask_cors import CORS, cross_origin  # noqa: E402
...
# Initialize CORS
# Only apply CORS in production/API-only mode
if (
    os.environ.get("FLASK_ENV") == "production"
    or os.environ.get("FLASK_API_ONLY") == "true"
):
    # Configure CORS
    CORS(
        app,
...
@app.route("/api/generate_invite_token", methods=["POST"])
@cross_origin()
def generate_invite_token():
    """
    Front-end sends user_id (and optionally other info).
    Back-end generates a secure, time-limited token, stores it in the user's document,
    and returns it. If a valid token already exists, return that token.
    """
    try:
        user_id = request.json.get("user_id")
        # You can add other info as needed

        if not user_id:
            return jsonify({"error": "Missing user id"}), 400

        # Check if user exists
        if not db.verify_user(user_id):
            app.logger.error("User %s not found", user_id)
            return jsonify({"error": "User not found"}), 404

        # Check for existing token
        token_info = db.get_invite_token_info_for_user(user_id)
        now = datetime.now(timezone.utc)
        if token_info and "token" in token_info and "expiration" in token_info:
            expiration = token_info["expiration"]
            # If expiration is a string, convert to datetime
            if isinstance(expiration, str):
                expiration = datetime.fromisoformat(expiration)
            # for testing purposes print now and expiration
            app.logger.info("Now: %s, Expiration: %s", now, expiration)
            if now < expiration:
                # Token is still valid
                # Also return the time left
                time_left = expiration - now
                # for testing purposes print out the token and time_left
                app.logger.info(
                    "Old Token: %s, Time left: %s",
                    token_info["token"],
        ...
        expiration = now + timedelta(hours=24)  # Token valid for 24 hours

        # Store token in DB
        db.store_invite_token(token, user_id, expiration.isoformat())

        # also return the time left
        time_left = expiration - now
        app.logger.info(
            "New Token: %s, Time left: %s", token, time_left.total_seconds()
        )
        return jsonify(
            {
                "token": token,
        ...
        app.logger.error("Error generating invite token")
        app.logger.error(e)
        return jsonify({"error": "Internal server error"}), 500
...
def token_login():
    ...
    Back-end checks if token is valid and not expired, logs in the user, and returns user_id.
    """
    try:
        ...
        # Look up token info
        token_info = db.get_invite_token_info(token)
        if not token_info:
        ...
        app.logger.info("User %s logged in via token", user_id)

        return jsonify({"status": "ok", "user_id": user_id})

    except Exception as e:
        app.logger.error("Error during token login")
        app.logger.error(e)
        return jsonify({"error": "Internal server error"}), 500
...
def log_in():
    ...
    Back-end returns {ok} if user_id is valid
    """
...
@login_required
def protected():
    return jsonify({"authenticated": True, "user_id": current_user.get_id()}), 200
...
def get_projects():
    """
    Returns the list of projects for the logged-in user.
    """
    try:
        ...
        app.logger.error("Error when fetching projects for user %s", user_id)
        app.logger.error(e)
        raise InternalServerError() from e
    ...
    try:
        ...
        if WRITE_TO_DB:
            ...
            db.edit_project_fields(
                project_id,
                {
                    "context_response": data["questions"],
...
def check_question_response():
    ...
    try:
        ...
        if context_question_id == 8:
            response_text = f"{
                response_text
            } percent of questions are open-ended and the remaining are close-ended"
        ...
        if len(advice) > 0:
            ...
            ignored_warnings = dspya.format_list_to_string(ignored_warnings)
            ...
            output = dspya.invoke_module(
                **{
                    "module_name": DSPyModule.CHECK_PROMPT,
            ...
            if int(output["inform_score"]) <= CHECK_PROMPT_THRESHOLD:
                warnings = dspya.format_string_to_list(
                    output["suggestions"], "suggestions"
                )

        app.logger.info(
            "Checked response for question %s in project %s",
            context_question_id,
            project_id,
        )
...
def update_context():
    ...
    try:
        ...
        app.logger.info(
            "Saving context for user %s project %s: %s", user_id, project_id, context
        )
        ...
        app.logger.error("Error when updating context for project %s", project_id)
        app.logger.error(e)
        raise InternalServerError() from e
...
def generate_draft(project_id):
    ...
    output = dspya.invoke_module_json_output(
        **{
    ...
    output = dspya.invoke_module(
        **{
            "module_name": DSPyModule.DETECT_TOPICS,
            "is_gpt4": True,
            "context": formatted_context,
    ...
    app.logger.info(
        "Generated the following topics for project %s: %s",
        project_id,
        output["topics"],
    )
    ...
    if WRITE_TO_DB:
        db.add_prompt(
            project_id, DSPyModule.DETECT_TOPICS.value, {"context": context}, output
        )
    ...
    for topic in topics:
        analyze_topics_info_topics[topic] = {"cells": [], "suggestion_rationale": ""}
...
def submit_context():
    ...
    try:
        ...
        app.logger.error("Error when submitting context for project %s", project_id)
        app.logger.error(e)
        raise InternalServerError() from e
...
def switch_response_format():
    ...
    try:
        ...
        formatted_question = dspya.format_cells(
            [new_question], ["time_estimate"], draft_type
        )
        ...
        app.logger.error("Error when switching response format for cell %s", cell_id)
        app.logger.error(e)
        raise InternalServerError() from e
...
def reword_question():
    ...
    try:
        ...
        cell_id = request.json["cell_id"]
        cell_details = request.json["cell_details"]
        existing_questions = request.json[
            "existing_questions"
        ]  # an object mapping cell_ids to cell_details for a section
        ...
        existing_questions_str = json.dumps(
            [existing_questions[cell_id] for cell_id in existing_questions]
        )
...
def generate_specific_rewording():
    ...
    try:
        ...
        app.logger.error("Error when rewording question %s from request", cell_id)
        app.logger.error(e)
        raise InternalServerError() from e
...
def check_question():
    ...
    try:
        ...
        scores = dspya.invoke_module(
            **{
                "module_name": DSPyModule.ASSESS_QUESTIONS,
        ...
        # format the cells
        rewritten_questions = dspya.format_cells(
            rewritten_questions, ["time_estimate"], draft_type
        )
        # app.logger.info("Rewritten questions: %s", rewritten_questions)
        ...
        return jsonify(
            {
                "check_suggestions": cleaned_rewritten_questions,
                "cell_checks": output["cleaned_scores"],
            }
        )
...
def get_over_and_under_topics(topics):
    ...
    over_topics = {
        topic: proportions[topic]
        for topic in proportions
        if proportions[topic] > min(2 / n_topics, 0.5)
    }
    under_topics = {
        topic: proportions[topic]
        for topic in proportions
        if proportions[topic] <= 0 and proportions[topic] < 1 / n_topics
    }
...
def generate_suggestion_rationale(
    suggestion_type, topic_name, current_proportion, num_delta=0, time_saved=0
):
    """
    ...
    if suggestion_type == "add":
        if current_proportion == 0:
            return (
                f'Currently there are no questions about "{topic_name}". '
                + f'You could increase the representation of this topic by adding some questions relating to "{topic_name}".'
            )
        else:
            return (
                f'Currently {current_proportion:.1%} of questions are about "{topic_name}". '
                + f'You could increase the representation of this topic by adding some questions relating to "{topic_name}".'
            )
...
def generate_summary_paragraph(num_topics, over_topics, under_topics):
    ...
    return (
        f"The visualization below displays the distribution of questions across {num_topic_content}. "
...
def analyze_topics():
    ...
    try:
        ...
        for topic in topics:
            new_topics[topic] = {"cells": [], "suggestion_rationale": ""}
                ...
                cell_details.append(
                    {
                        ...
                        "pre_selected_topics": human_topics[cell_id]
                        if cell_id in human_topics
                        else [],
                    }
                )
        ...
        # if the topics were edited, but no cells were edited, run classification on new topics
            ...
            for cell in cells:
                ...
                pre_selected_topics = (
                    human_topics[cell["cell_id"]]
                    if cell["cell_id"] in human_topics
                    else []
                )
                cell_details.append(
                    {
                        ...
                        "pre_selected_topics": list(
                            set.intersection(
                                set(added_topics), set(pre_selected_topics)
                            )
                        ),
                    }
                )
                    ...
                    cell_details.append(
                        {
                            ...
                            "cell_details": cell["cell_details"],
                            "all_topics": topics_lst,
                            "pre_selected_topics": human_topics[cell["cell_id"]]
                            if cell["cell_id"] in human_topics
                            else [],
                        }
                    )
        ...
        if len(cell_details) > 0:
            ...
            def classify_topics(cell):
                ...
                pre_selected_topics_str = dspya.format_list_to_string(
                    cell["pre_selected_topics"]
                )

                app.logger.info(
                    "Classifying topics for question %s",
                    cell["cell_details"]["main_text"],
                )
                ...
                # invoke the module and get the output
                output = dspya.invoke_module_multiple_times(
                    **{
                        "module_name": DSPyModule.CLASSIFY_TOPICS,
                ...
                # clean output
                cell_topics = dspya.clean_outputs_from_topic_classification(
                    output, cell["all_topics"], cell["pre_selected_topics"]
                )
                ...
                app.logger.info(
                    "Classified topics for question %s: %s",
                    cell["cell_details"]["main_text"],
                    cell_topics,
                )
        ...
        def generate_suggestions(topic):
            if topic in under_topics:
                ...
                new_topics[topic]["suggestion_rationale"] = (
                    generate_suggestion_rationale(
                        "add",
                        topic,
        ...
        summary_paragraph = generate_summary_paragraph(
            num_topics, over_topic_names, under_topic_names
        )

        # get the old suggestions from the projects collection
        old_suggestions = db.get_field_in_project(project_id, "analyze_topics_info")[
            "suggestions"
        ]
        ...
        app.logger.info(
            "Execution time for analyzing topics for project %s: %s",
            project_id,
            execution_time,
        )
...
def save_analyze_topics_info():
    ...
    try:
        ...
        if WRITE_TO_DB:
            ...
            app.logger.info(
                "Finished saving analyze topics info for project %s", project_id
            )
        ...
        app.logger.error(
            "Error when saving analyze topics info for project %s", project_id
        )
        app.logger.error(e)
        raise InternalServerError() from e
...
def get_delete_suggestions():
    ...
    try:
        ...
        questions_to_remove = dspya.map_cell_detail_to_cell_id(
            json.loads(output["questions_to_remove"]),
            existing_questions,
            external_fields=["rationale"],
        )

        app.logger.info(
            "Questions to remove for topic %s: %s", topic, questions_to_remove
        )

        # prepare the response
        response = {"suggestion_type": "delete", "suggestions": questions_to_remove}

        return jsonify({"content": response})

    except Exception as e:
        app.logger.error(
            "Error when getting delete suggestions for project %s topic %s",
            project_id,
            topic,
        )
        app.logger.error(e)
...
def get_add_suggestions():
    ...
    try:
        ...
        app.logger.info(
            "Getting add suggestions for project %s topic %s", project_id, topic
        )
        ...
        # convert to json and convert to list of cell_ids
        additional_questions = dspya.format_cells(
            json.loads(output["additional_questions"]),
            ["time_estimate", "rationale", "section_id"],
            draft_type,
        )

        app.logger.info(
            "Questions to add for topic %s: %s", topic, additional_questions
        )

        # prepare the response
        response = {"suggestion_type": "add", "suggestions": additional_questions}

        return jsonify({"content": response})

    except Exception as e:
        app.logger.error(
            "Error when getting add suggestions for project %s topic %s",
            project_id,
            topic,
        )
        app.logger.error(e)
...
def export_project(project_id):
    ...
    Front-end sends project_id in the URL
    Back-end returns the project data as a downloadable JSON file
    """
    try:
        ...
        if project_id not in project_ids:
            app.logger.error(
                "User %s does not have access to project %s", user_id, project_id
            )
            raise NotFound()

        # Format the project for export
        project_data = db.format_project_for_export(project_id)
        ...
        # Limit title to first 5 words to avoid overly long filenames
        words = project_title.split()
        truncated_title = " ".join(words[:5])
        safe_title = "".join(
            c for c in truncated_title if c.isalnum() or c in (" ", "_", "-")
        ).replace(" ", "_")
        ...
        app.logger.info("Exported project %s for user %s", project_id, user_id)

        return response

    except Exception as e:
        app.logger.error("Error when exporting project %s", project_id)
        app.logger.error(e)
        raise InternalServerError() from e
        ...
        if WRITE_TO_DB:
            ...
            for project_id, event_list in events.items():
                if len(event_list) > 0:
                    ...
                    app.logger.info(
                        "Updated events for user %s project %s", user_code, project_id
                    )

        return jsonify({"status": "ok"})

    except Exception as e:
        app.logger.error("Error when updating events for user %s", user_code)
        app.logger.error(e)
        raise InternalServerError() from e
...
if __name__ == "__main__":
    ...
    # NOTE: change back to port 5000 for docker
